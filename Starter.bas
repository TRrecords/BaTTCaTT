...
Sub device_BatteryChanged (Level As Int, Scale As Int, Plugged As Boolean, Intent As Intent)
	...
	Next
		...
		sNotif.Icon="batusb"
		sNotif.Sound=False
		sNotif.SetInfo("USB laden: "&Level&" %",volt&" V | "&temp&"°C | noch: "&hours&"h,"&minutes&"min",Main)
		sNotif.Notify(1)
	...
	Else
		...
		minutes = val Mod 60
		If  Not (Level=0) Then 
			sNotif.SetInfo("USB laden: "&Level&" %",volt&" V | "&temp&"°C | noch: "&hours&"h und "&minutes&"min",Main)
			sNotif.Notify(1)
		End If
	If Level <= 100 Then
		'kvs2.PutSimple(Level,time)
